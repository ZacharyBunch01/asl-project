...
def test_categorical_skips_missing_columns(monkeypatch, tmp_path):
    ...
    # Fake save_plot that just records its arguments instead of writing files
    def fake_save_plot(fig, name, folder="../Figures"):
        calls.append((fig, name, folder))

    # Patch save_plot in the categorical module to use the fake version
    ...
    # Run plotting with no matching features
...
def test_categorical_uses_default_features(monkeypatch, tmp_path):
    # When default features are present, save_plot should be called once per feature
    calls = []
...
def test_categorical_custom_features_and_top_n(monkeypatch, tmp_path):
       # With a custom feature list and top_n, save_plot should be called only for existing features
    calls = []
